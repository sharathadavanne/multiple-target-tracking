...
% History:
%   05.02.2008  The first official version.
%
...
function [S,ev_strs] = ukf_nmcda_update(S,Y,t,h,R,CP,CD,pb,alpha,beta,param)
  ...
  for i=1:length(S)
    ...
    for j=1:length(S{i}.M)
      ...
      %
      [M,P,K,IM,IS,lhood] = ...
	  ukf_update1(S{i}.M{j},S{i}.P{j},Y,h,R,param);
    ...
    [M,P,K,IM,IS,lhood] = ...
	ekf_update(m,S{i}.P0,Y,H,R,h,V,param);
