%UKF_NMCDA_UPDATE_SA  UKF/NMCDA Update step (single DA per target)
%
% Syntax:
%   [S,EV_STRS] = ukf_nmcda_update_sa(S,Y,time,h,R,pd,CD,pb,alpha,beta,param)
%
% Author:
%   Simo Särkkä, 2005
%
...
%
% See also:
%   NMCDA_INIT, UKF_NMCDA_PREDICT, UKF_NMCDA_UPDATE
...
%
% $Id: ukf_nmcda_update_sa.m, $
%
% This software is distributed under the GNU General Public
% Licence (version 2 or later); please refer to the file
% Licence.txt, included with the software, for details.

function [SS,ev_strs] = ukf_nmcda_update_sa(S,Y,time,h,R,pd,CD,pb,alpha,beta,param)
    ...
    for i=1:length(S)
        ...
        for j=1:length(S{i}.M)
            if D{i}(j) == 0 % Not yet detected
                ...
                %
                [M,P,K,IM,IS,lhood] = ...
                    ukf_update1(S{i}.M{j},S{i}.P{j},Y(:,m),h,R,param);
        ...
        % New target
        [M,P,K,IM,IS,lhood] = ...
            ukf_update(S{i}.M0,S{i}.P0,Y(:,m),h,R,param);
        ...
        %
        [M,P,K,IM,IS,lhood] = ...
            ukf_update1(S{i}.M0,S{i}.P0,Y(:,m),h,R,param);
