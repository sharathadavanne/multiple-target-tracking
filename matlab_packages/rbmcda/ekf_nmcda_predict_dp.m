%EKF_NMCDA_PREDICT_DP  EKF/NMCDA Prediction step with target death processing
%
% Syntax:
%   [S,W,EV_STRS] = ekf_nmcda_predict_dp(S,W,A,Q,a,AW,param,t,alpha,beta)
%
% In:
%   S  - Struct array 1xNP of particles
%   W  - Importance weights as 1xNP vector
...
%   W - Predicted importance weights
...
function [S,ev_strs] = ekf_nmcda_predict_dp(S,A,Q,a,AW,param,t,alpha,beta)
%
% Check arguments
%
if nargin < 2
...
for i=1:length(S)
    ...
    for j=1:length(S{i}.M)
        ...
        if j ~= dead
            ...
            [S{i}.M{j},S{i}.P{j}] = ekf_predict1(S{i}.M{j},S{i}.P{j},A,Q,a,AW,param);
        end
    end
