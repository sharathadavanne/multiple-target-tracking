%EKF_NMCDA_UPDATE_SA  EKF/NMCDA Update step (single DA per target)
%
% Syntax:
%   [SS,EV_STRS] = ekf_nmcda_update_sa(S,Y,time,H,R,h,V,pd,CD,pb,alpha,beta,param)
%
% Author:
%   Simo Särkkä, 2005
%
...
%
% See also:
%   NMCDA_INIT, EKF_NMCDA_PREDICT, EKF_EKF_NMCDA_UPDATE
...
%
% $Id: ekf_nmcda_update_sa.m, $
%
% This software is distributed under the GNU General Public
% Licence (version 2 or later); please refer to the file
% Licence.txt, included with the software, for details.

function [SS,ev_strs] = ekf_nmcda_update_sa(S,Y,time,H,R,h,V,pd,CD,pb,alpha,beta,param)
    ...
    for i=1:length(S)
        ...
        for j=1:length(S{i}.M)
            if D{i}(j) == 0 % Not yet detected
                ...
                %
                [M,P,K,IM,IS,lhood] = ...
                    ekf_update1(S{i}.M{j},S{i}.P{j},Y(:,m),H,R,h,V,param);
        ...
        % New target
        [M,P,K,IM,IS,lhood] = ...
            ekf_update(S{i}.M0,S{i}.P0,Y(:,m),H,R,h,V,param);
        ...
        %
        [M,P,K,IM,IS,lhood] = ...
            ekf_update1(S{i}.M0,S{i}.P0,Y(:,m),H,R,h,V,param);
